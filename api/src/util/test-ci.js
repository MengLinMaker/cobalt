...
import { Bright, Red } from '../misc/console-text.js'
  ...
  case 'run-tests-for': {
    ...
    for (const test of tests[service]) {
      ...
      try {
        ...
        const failText = canFail ? `${Red('FAIL')} (ignored)` : Bright(Red('FAIL'))
        if (canFail && process.env.GITHUB_ACTION) {
          console.log(
            `::warning title=${service}/${name.replace(/,/g, ';')}::failed and was ignored`,
          )
        }
