...
function com_resolveShortlink(shortId) {
  return fetch(`https://b23.tv/${shortId}`, { redirect: 'manual' })
    ...
    .then((url) => {
      ...
      if (path.startsWith('/video/')) return path.split('/')[2]
    })
    .catch(() => {})
}
...
async function com_download(id) {
  ...
  const streamData = JSON.parse(html.split('<script>window.__playinfo__=')[1].split('</script>')[0])
...
async function tv_download(id) {
  ...
  const [video, audio] = extractBestQuality({
    video: data.playurl.video.map((s) => s.video_resource).filter((s) => s.codecs.includes('avc1')),
