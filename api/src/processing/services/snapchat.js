...
const SPOTLIGHT_VIDEO_REGEX =
  /<link data-react-helmet="true" rel="preload" href="([^"]+)" as="video"\/>/
...
async function getStory(username, storyId, alwaysProxy) {
  ...
  if (nextDataString) {
    ...
    if (storyIdParam && data.props.pageProps.story) {
      const story = data.props.pageProps.story.snapList.find(
        (snap) => snap.snapId.value === storyIdParam,
      )
      if (story) {
