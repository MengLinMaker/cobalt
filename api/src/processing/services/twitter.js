...
const tweetFeatures = JSON.stringify({
...
function needsFixing(media) {
  ...
  const mediaTimestamp = new Date(Number((BigInt(representativeId) >> 22n) + TWITTER_EPOCH))
...
function bestQuality(arr) {
  return arr
    ...
    .reduce((a, b) => (Number(a?.bitrate) > Number(b?.bitrate) ? a : b)).url
}
    ...
    default:
      ...
      const picker = media.map((content, i) => {
        if (content.type === 'photo') {
          ...
          const proxiedImage = proxyThumb(url, i)
