...
const requestApiInfo = (videoId, password) => {
  ...
  return fetch(`https://api.vimeo.com/videos/${videoId}`, {
    headers: {
      ...
      'User-Agent':
        'Vimeo/10.19.0 (com.vimeo; build:101900.57.0; iOS 17.5.1) Alamofire/5.9.0 VimeoNetworking/5.0.0',
      Authorization:
...
const compareQuality = (rendition, requestedQuality) => {
  const quality = Number.parseInt(rendition)
...
const getHLS = async (configURL, obj) => {
  ...
  if (obj.quality < resolutionMatch[variants[0]?.resolution?.width]) {
    bestQuality = variants.find((v) => obj.quality === resolutionMatch[v.resolution.width])
  }
