export const testers = {
  bilibili: (pattern) =>
    pattern.comId?.length <= 12 || pattern.comShortLink?.length <= 16 || pattern.tvId?.length <= 24,
  ...
  loom: (pattern) => pattern.id?.length <= 32,

  ok: (pattern) => pattern.id?.length <= 16,
  ...
  streamable: (pattern) => pattern.id?.length === 6,

  tiktok: (pattern) => pattern.postId?.length <= 21 || pattern.id?.length <= 13,

  tumblr: (pattern) =>
    pattern.id?.length < 21 || (pattern.id?.length < 21 && pattern.user?.length <= 32),

  twitch: (pattern) => pattern.channel && pattern.clip?.length <= 100,

  twitter: (pattern) => pattern.id?.length < 20,

  vimeo: (pattern) =>
    pattern.id?.length <= 11 && (!pattern.password || pattern.password.length < 16),

  vine: (pattern) => pattern.id?.length <= 12,

  vk: (pattern) => pattern.userId?.length <= 10 && pattern.videoId?.length <= 10,

  youtube: (pattern) => pattern.id?.length <= 11,
  ...
  bsky: (pattern) => pattern.user?.length <= 128 && pattern.post?.length <= 128,
}
