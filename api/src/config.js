...
const env = {
  ...
  rateLimitWindow:
    (process.env.RATELIMIT_WINDOW && Number.parseInt(process.env.RATELIMIT_WINDOW)) || 60,
  rateLimitMax: (process.env.RATELIMIT_MAX && Number.parseInt(process.env.RATELIMIT_MAX)) || 20,

  durationLimit:
    (process.env.DURATION_LIMIT && Number.parseInt(process.env.DURATION_LIMIT)) || 10800,
  streamLifespan:
    (process.env.TUNNEL_LIFESPAN && Number.parseInt(process.env.TUNNEL_LIFESPAN)) || 90,

  processingPriority:
    process.platform !== 'win32' &&
    process.env.PROCESSING_PRIORITY &&
    Number.parseInt(process.env.PROCESSING_PRIORITY),
  ...
  sessionEnabled:
    process.env.TURNSTILE_SITEKEY && process.env.TURNSTILE_SECRET && process.env.JWT_SECRET,
...
export { env, genericUserAgent, cobaltUserAgent }
