...
const validateKeys = (input) => {
  ...
  Object.values(input).map((details) => {
    ...
    if (details.limit && details.limit !== 'unlimited') {
      ...
      if (details.limit < 1) throw 'detail object contains invalid limit (not a positive number)'
    }
  ...
  for (let key in keyData) {
    ...
    if (data.limit) {
      if (data.limit === 'unlimited') {
        data.limit = Number.POSITIVE_INFINITY
      }
...
const loadKeys = async (source) => {
  ...
  if (source.protocol === 'file:') {
    ...
    updated = JSON.parse(await readFile(decodeURIComponent(source.host + pathname), 'utf8'))
  } else {
...
export const validateAuthorization = (req) => {
  ...
  if (matchingKey.ips) {
    ...
    const ip_allowed = matchingKey.ips.some(([allowed, size]) => {
      return addr.kind() === allowed.kind() && addr.match(allowed, size)
    })
